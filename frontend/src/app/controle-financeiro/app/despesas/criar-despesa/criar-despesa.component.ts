...
export class CriarDespesaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.formulario.get('valor')?.valueChanges.subscribe(
      valorDigitado => {
        ...
        if (valorDigitado.toString().split(".")[0].length > MAX_INT_LEN) {
          this.formulario.get('valor')?.setValue(valorDigitado.toString().slice(0, MAX_INT_LEN));
        }

        // Check if the value has more than 2 decimal places, if so, round it to 2 decimal places
        if (valorDigitado && valorDigitado.toString().split(".")[1]?.length > MAX_DECIMAL_LEN) {
